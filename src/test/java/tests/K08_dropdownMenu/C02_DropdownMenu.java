...
public class C02_DropdownMenu extends TestBase_Each {
    ...
    public void test01() {
        ...
        // 1- Doğum tarihi gun seceneginden index kullarak 5'i secin
        ...
        WebElement ddmGun = driver.findElement(By.xpath("(//select[@class ='form-control'])[1]"));
            ...
            //          kullanmak istedigini dropdown menuyu girin
        Select selectGun = new Select(ddmGun);

            // 3.adim : olusturdugunuz selectGun objesi sayesinde
            //          Select class'indaki hazir methodlar ile istenen islemleri yapabiliriz
        selectGun.selectByIndex(5);

        // 2- Doğum tarihi ay seceneginden value olarak Nisan'i secin
        ...
        // 5- Ay dropdown menudeki tum degerleri (value) yazdirin
        ...
        List<String> ayDdmYazilarList = new ArrayList<>();
        List<WebElement> ayDdmElementleriList = selectAy.getOptions();

        for (WebElement each : ayDdmElementleriList) {
            ayDdmYazilarList.add(each.getText());
            
        }
        System.out.println(ayDdmYazilarList);

        Assertions.assertTrue(ayDdmYazilarList.contains("Ocak"));

        //3.yontem : ReusableMethods class'indaki meethodu kullanalim
